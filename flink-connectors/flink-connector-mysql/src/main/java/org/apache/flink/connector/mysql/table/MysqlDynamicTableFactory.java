...
package org.apache.flink.connector.mysql.table;

import org.apache.flink.annotation.Internal;
import org.apache.flink.configuration.ConfigOption;
import org.apache.flink.configuration.ConfigOptions;
import org.apache.flink.configuration.ReadableConfig;
import org.apache.flink.connector.mysql.JdbcExecutionOptions;
import org.apache.flink.connector.mysql.dialect.JdbcDialect;
import org.apache.flink.connector.mysql.dialect.MySQLDialect;
import org.apache.flink.connector.mysql.dialect.MySQLDialect2;
import org.apache.flink.connector.mysql.internal.options.JdbcDmlOptions;
import org.apache.flink.connector.mysql.internal.options.JdbcLookupOptions;
import org.apache.flink.connector.mysql.internal.options.JdbcOptions;
import org.apache.flink.connector.mysql.internal.options.JdbcReadOptions;
...
public class MysqlDynamicTableFactory
    ...
    // read config options
    private static final ConfigOption<String> SCAN_PARTITION =
            ConfigOptions.key("scan.partition")
                    .stringType()
                    .noDefaultValue()
                    .withDescription("the Mysql partition.");
    ...
    private static final ConfigOption<Boolean> SINK_ON_UPDATE =
            ConfigOptions.key("sink.no-update").booleanType().defaultValue(false);
    private static final ConfigOption<Boolean> SINK_ON_DELETE =
            ConfigOptions.key("sink.no-delete").booleanType().defaultValue(false);
    ...
    private JdbcOptions getJdbcOptions(ReadableConfig readableConfig) {
        ...
        final Boolean noUpdate = readableConfig.get(SINK_ON_UPDATE);
        JdbcDialect dialect = noUpdate ? new MySQLDialect2() : new MySQLDialect();
        //        JdbcDialect dialect = new MySQLDialect2();
    ...
    private JdbcReadOptions getJdbcReadOptions(ReadableConfig readableConfig) {
        final Optional<String> partitionName = readableConfig.getOptional(SCAN_PARTITION);
    ...
    private JdbcExecutionOptions getJdbcExecutionOptions(ReadableConfig config) {
        ...
        builder.withNoUpdate(config.get(SINK_ON_UPDATE));
        builder.withNoDelete(config.get(SINK_ON_DELETE));
        return builder.build();
    }
    ...
    public Set<ConfigOption<?>> optionalOptions() {
        ...
        optionalOptions.add(SCAN_PARTITION);
        ...
        optionalOptions.add(SINK_ON_UPDATE);
        optionalOptions.add(SINK_ON_DELETE);
        return optionalOptions;
    }

    private void validateConfigOptions(ReadableConfig config) {
        String jdbcUrl = config.get(URL);
        final Optional<JdbcDialect> dialect = Optional.of(new MySQLDialect());
