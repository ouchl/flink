 ...
 */
public final class TableBufferReducedStatementExecutor implements VerticaBufferedBatchStatementExecutor<RowData> {
	...
	public void prepareStatements(Connection connection) throws SQLException {
		...
		mergeExecutor.prepareStatements(connection);
	...
	public void executeBatch() throws SQLException {
		for (Map.Entry<RowData, Tuple2<Boolean, RowData>> entry : reduceBuffer.entrySet()) {
			if (entry.getValue().f0) {
				mergeExecutor.addToBatch(entry.getValue().f1);
			} else {
				// delete by key
